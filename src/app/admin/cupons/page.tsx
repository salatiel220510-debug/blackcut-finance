...
export default async function CuponsPage() {
  ...
  const [sessoesFechadas, transacoesIncome, transacoesExpense] = await Promise.all([
  ...
  function paraView(t: (typeof transacoesIncome)[number] | (typeof transacoesExpense)[number]) {
    return {
      id: t.id, type: t.type, category: t.category, description: t.description, observacao: t.observacao,
      amount: Number(t.amount), date: t.date.toISOString(), barberNome: t.barber?.name ?? null,
      commissionAmount: t.commissionAmount ? Number(t.commissionAmount) : null, paymentMethod: t.paymentMethod,
      clienteNome: t.clienteNome, comandaId: t.comandaId, criadoPorNome: "", podeExcluir: false,
    };
  }

  const vendasAgrupadas = agruparPorComanda(transacoesIncome.map(paraView));
  const gastosAgrupados = agruparPorComanda(transacoesExpense.map(paraView));

  const cuponsFechamento = sessoesComPrevia.map((s) => ({
  ...
  const cuponsVendaGasto = [
    ...vendasAgrupadas.map((c) => ({
      tipo: "venda" as const,
      id: c.chave,
      data: c.date,
      ...
      nome: c.itens[0]?.clienteNome ?? null,
      barberNome: c.barberNome,
    })),
    ...gastosAgrupados.map((c) => ({
      ...
      total: c.amount,
      paymentMethod: c.itens[0]?.paymentMethod ?? null,
      nome: c.itens[0]?.description ?? null,
      barberNome: c.barberNome,
    })),
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <main className="flex-1 max-w-3xl w-full mx-auto px-4 py-8">
        <h1 className="font-display text-2xl text-gold mb-1">Cupons e Horas do Mês</h1>
        <p className="text-gray-400 text-sm mb-6">
          {agora.toLocaleDateString("pt-BR", { month: "long", year: "numeric" })} — reinicia automaticamente todo mês.
        </p>
        ...
        <section className="border border-gold-dark/40 bg-black-soft rounded-xl p-5">
          <h2 className="font-display text-lg text-gold mb-4">Cupons</h2>
          <ListaCupons cuponsFechamento={cuponsFechamento} cuponsVendaGasto={cuponsVendaGasto} />
        </section>
      </main>
      <Footer role={role} />
    </div>
